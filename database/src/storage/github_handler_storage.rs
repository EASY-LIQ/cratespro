...
use entity::{
    contributor_location, crates, github_sync_status, github_user, metadata,
    programs::{self},
...
impl GithubHanlderStorage {
    ...
    pub async fn get_metadata_stream(
        ...
        let mut condition = Condition::all();
        if !update_all {
            //获取MetadataUpdateState为false的记录（没有查询过的数据）
            condition = condition.add(programs::Column::MetadataUpdateState.eq(false));
        }
        programs::Entity::find()
            .filter(condition)
            .filter(programs::Column::InCratesio.eq(true))
            .order_by_asc(programs::Column::Id)
            .stream(self.get_connection())
            .await
    }
    pub async fn save_metadata_model(&self, models: metadata::ActiveModel) -> Result<(), DbErr> {
        metadata::Entity::insert(models)
            .on_conflict(
                OnConflict::column(metadata::Column::Id)
                    .update_columns([
                        metadata::Column::UpdatedAt,
                        metadata::Column::IsArchived,
                        metadata::Column::LicenseName,
                        metadata::Column::DiskUsage,
                        metadata::Column::StargazerCount,
                        metadata::Column::ForkCount,
                        metadata::Column::WatcherCount,
                        metadata::Column::MentionableUserCount,
                        metadata::Column::OpenIssues,
                        metadata::Column::ClosedIssues,
                        metadata::Column::OpenPullRequests,
                        metadata::Column::ClosedPullRequests,
                        metadata::Column::MergedPullRequests,
                        metadata::Column::CommitCount,
                        metadata::Column::PushedAt,
                        metadata::Column::PrimaryLanguage,
                        metadata::Column::ReleaseCount,
                        metadata::Column::OwnerType,
                        metadata::Column::LanguageTotalCount,
                        metadata::Column::LanguageTotalSize,
                        metadata::Column::LanguagesJson,
                    ])
                    .to_owned(),
            )
            .exec(self.get_connection())
